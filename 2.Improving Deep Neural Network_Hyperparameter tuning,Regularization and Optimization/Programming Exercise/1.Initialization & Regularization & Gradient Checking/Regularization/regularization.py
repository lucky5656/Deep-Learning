...
def backward_propagation_with_regularization(X, Y, cache, lambd):
    ...
    dA2 = np.dot(W3.T, dZ3)
    dZ2 = np.multiply(dA2, np.int64(A2 > 0))  # np.int64(A2 > 0)是与A2大小对应的值全为1的矩阵，即relu函数的导数
    dW2 = (1 / m) * np.dot(dZ2, A1.T) + ((lambd * W2) / m)
    db2 = (1 / m) * np.sum(dZ2, axis=1, keepdims=True)

    dA1 = np.dot(W2.T, dZ2)
    dZ1 = np.multiply(dA1, np.int64(A1 > 0))    # np.int64(A1 > 0)是与A1大小对应的值全为1的矩阵，即relu函数的导数
...
def forward_propagation_with_dropout(X, parameters, keep_prob=0.5):
    ...
    D1 = D1 < keep_prob  # 步骤2：将D1的值转换为0或1（使用keep_prob作为阈值） ;若D1低于 (keep_prob)设置为0，高于(keep_prob)设置为1
...
def backward_propagation_with_dropout(X, Y, cache, keep_prob):
    ...
    dZ2 = np.multiply(dA2, np.int64(A2 > 0))  # 此语句作为relu的导数
